...
void	__child(t_parse *cmd, t_env **env)
{
	...
	while (cmd->next)
	{
		...
		if (i > 0)
			close(in);
		in = fd[0]; // save the pipe readend from this command to inherit it to the next command
		cmd = cmd->next;
		i++;
	}
}
...
void	supervisor(void)
{
	...
	status = 0;
	g_vars.exit_sig = 0;
	waitpid(g_vars.pid, &status, 0); //get last child exit status
	while (wait(NULL) > 0) // wait for all childs
		;
	if (WIFEXITED(status))
...
void	exec_pipeline(t_parse *commands, t_env **env)
{
	...
	head = commands;
	if (head && head->cmd) // single builtins command executed by the shell itself
	{
		if (simple_cmd(head))
		{
